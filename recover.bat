...
setlocal
cd /d "%~dp0"
title Swim Rankings - Recover Missing

echo ============================================
echo   SWIM RANKINGS - RECOVER MISSING DOWNLOADS
echo ============================================
echo.
echo   Run this AFTER a weekly refresh if some clubs failed
echo   (e.g. 502 errors). It KEEPS what you already have and
echo   only re-downloads the missing files, then re-uploads.
echo.
...
if not exist "%CHROME%" (
  echo ERROR: Could not find Chrome. Edit recover.bat and set the CHROME path.
  pause
  exit /b 1
)
...
echo.
echo   Then press any key here to start the recovery...
pause >nul

rem NOTE: no clearing here - scrape-cdp.js resumes and skips files already
rem downloaded, so only the missing/failed ones are fetched.

echo.
echo [1/3] Downloading only the missing files...
...
echo.
echo ============================================
echo   DONE. The website will show the data
